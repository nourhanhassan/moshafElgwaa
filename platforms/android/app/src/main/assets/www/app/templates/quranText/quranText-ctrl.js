app.controller('quranTextController', function ($scope, $window, $ionicSlideBoxDelegate, moshafdata, ionicPopup, $filter, $q, $state, $stateParams, $timeout, mark, $ionicPopover, enums, target, memorize, $ionicGesture, $rootScope, $ionicScrollDelegate, $ImageCacheFactory, device, $ionicSideMenuDelegate, audio, $ionicModal, localStorage, notes, bookmarkTypes, bookmarks, memorizedayat, toast, settings, ayatTafseer, ayatData, recitations, reviewTarget, review) {
    ...
    $scope.init = function () {
        ...
        $scope.zoomingFontSize = 6;
        $scope.zoomingNameFontSize = 5.7;
        $scope.zoomingNumFontSize = 3;
        $scope.paddingSize = 2;
        $scope.lineHeigh = 1.5;
    ...
    $scope.goToPage = function (pageNum, ayaNumStart, ayaNumEnd, repeatMode, repeatCount) {
        ...
        if (MoshafId == enums.MoshafId.hafstext) {
            $state.go("tab.quranText", {  page: pageNum, aya: ayaNumStart, repeatMode: repeatMode, ayaEnd: ayaNumEnd, repeatCount: repeatCount});
        }
        else {
                                ...
                                if ($stateParams.page == p.pageNum && $stateParams.aya > 0) {
                                    ...
                                    for (var i = 0; i < p.surahs.length; i++) {
                                        if ($scope.currentAya == null)
                                            var res = $filter('filter')(p.surahs[i].ayat, { id: parseInt($stateParams.aya) }, true);
                                           // var res = p.surahs[i].ayat.filter(function (obj) { return obj.id == parseInt($stateParams.aya) });
                                        if (res.length > 0) {
                                            res[0].isMarked = true;
                                            var aya = res[0];
                                            $timeout(function () {
    ...
    $scope.increaseRepeat = function () {
        ...
        else {    $scope.audioSettings.repeat = 10;
        }
    }
    ...
    $scope.zoomIn = function () {
        if ($scope.zoomingFontSize < 20) {
            $scope.zoomingFontSize += 1;
            $scope.zoomingNameFontSize += 1;
            $scope.zoomingNumFontSize += 1;
            $scope.paddingSize += 1;
            $scope.lineHeigh += 0.035;
        }
    }
    $scope.zoomOut = function () {
        if ($scope.zoomingFontSize > 6) {
            $scope.zoomingFontSize -= 1;
            $scope.zoomingNameFontSize -= 1;
            $scope.zoomingNumFontSize -= 1;
            $scope.paddingSize -= 1;

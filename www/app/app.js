...
app.run(function ($ionicPlatform, localStorage, $rootScope, $ionicNavBarDelegate, $stateParams, $cordovaNetwork, $state, ionicLoading, device, localNotification, $cordovaLocalNotification, contacts, target, memorizedayat, enums, moshafdb, mark, moshafdata, ayatData, $q, downloadSoarImage, $timeout, file, $ionicHistory, settings, ayatTafseer, audio, reviewTarget, file, fileTransfer, network, $ionicLoading, moshafdbShmr, moshafdbKalon, $ionicModal, moshafdbwarsh, pushNotification) {
    ...
    $rootScope.getArabicNumber = function (num) {
                ...
                if (targetData.targetMode == enums.targetMode.werd) {
                    ...
                    $timeout(function () {
                        ...
                        if (MoshafId == enums.MoshafId.hafstext) {
                            $state.go("tab.quranText", { target: targetData.id });
                        }
                        else {
                    ...
                    if (MoshafId == enums.MoshafId.hafstext) {
                        $state.go("tab.quranText", { reviewTarget: targetData.id });
                    }
                    else {
                        //settings.settingsData.MoshafId = enums.MoshafId.hafs;
                        $state.go("tab.page", { reviewTarget: targetData.id });
                    }
                    //$state.go("tab.page", {
        ...
        $ionicPlatform.registerBackButtonAction(function () {
            ...
            if ($state.current.name == "dashboard") {
                ...
                if ($ionicHistory.backView().stateName == "dashboard")
                    $state.go("dashboard");
                else
                    $state.go("tab.page"); 
            ...
            else if ($state.current.name == "target-list") {
                $state.go("tab.memo");
            }
            else {
        ...
        $rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
            ...
            if (toState.name == "register") $rootScope.fromState = fromState.name;
            audio.stop();
        });
    })
